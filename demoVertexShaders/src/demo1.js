...
let scene, camera, renderer, container, torus;
...
function buildTorus() {
	...
	customMaterial.onBeforeRender = (renderer, scene, camera, geometry, mesh) => {
		...
		uniforms.modelMatrix.value = torus.matrixWorld;
	...
	torus = new THREE.Mesh(new THREE.TorusGeometry(0.5, 0.2, 16, 100), customMaterial);
...
function animate() {
	...
	// animamos	la rotación del toroide
	torus.rotation.x += 0.01;
	torus.rotation.y += 0.014;
